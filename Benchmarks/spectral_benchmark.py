...
def params(t, patterned_material, wavelengths, lam, period, blur):
    device = (
        "cuda"
    ...
    options = {
            ...
            "mat 2": [patterned_material.eps(i) for i in wavelengths], # permittivity when density is 1
...
def run_pol_dependent_spec_benchmark(lam, patterned_material, t, wavelengths, targets, targetp, 
    ...
    device, seed, options, sim_dtype, geo_dtype = params(t, patterned_material, wavelengths, lam, period, blur)

    # Neural Network
    t = time.time()
    NN_des, NN_cost, NN_des_hist, NN_ts, NN_tp  = NN_pol_dep(seed, wavelengths, 
                                                             targets, targetp, 
                                                             layers, options, 
    ...
    for bl in blur:
        ...
        t = time.time()
        LMpx_des, LMpx_cost, LMpx_des_hist, LMpx_ts, LMpx_tp = LMpx_pol_dep(seed, wavelengths, 
                                                                            targets, targetp, 
                                                                            layers, options, 
                                                                            sim_dtype, geo_dtype, device)
    ...
    for bl in blur:
        options["blur radius"] = bl
        t = time.time()
        px_des, px_cost, px_des_hist, px_ts, px_tp = px_pol_dep(seed, wavelengths, 
                                                                            targets, targetp, 
                                                                            layers, options, 
                                                                            sim_dtype, geo_dtype, device)
    ...
    # NN + pixel approach
    t = time.time()
    NN_px_des, NN_px_cost, NN_px_des_hist, NN_px_ts, NN_px_tp  = NN_px_pol_dep(seed, wavelengths, 
                                                                               targets, targetp, 
                                                                               layers, options, 
                                                                               sim_dtype, geo_dtype, device)
    ...
    if plot:
        ...
        compare_performances_dual_pol_spectral(NN_ts, NN_tp, NN_px_ts, NN_px_tp, LMpx_tss, LMpx_tps, px_tss, px_tps, targets, 
                                               targetp, wavelengths, blur, f"{res_folder}{label}_performance_compare.png")
...
def run_pol_ind_spec_benchmark(lam, patterned_material, t, wavelengths, target, pol, 
                              period, layers, label, res_folder, blur = [None], plot = True):
    """
    lam - wavelength in free space (nm)
    patterend_material - Material class for patterned material
    ...
    # Neural Network
    t = time.time()
    NN_des, NN_cost, NN_des_hist, NN_t  = NN_no_pol(seed, wavelengths, 
                                                    target, pol, 
                                                    layers, options, 
                                                    sim_dtype, geo_dtype, device)
    NN_time = time.time() - t
    ...
    for bl in blur:
        options["blur radius"] = bl
        t = time.time()
        LMpx_des, LMpx_cost, LMpx_des_hist, LMpx_t = LMpx_no_pol(seed, wavelengths, 
                                                                            target, pol, 
                                                                            layers, options, 
                                                                            sim_dtype, geo_dtype, device)
        LMpx_times.append(time.time() - t)
    ...
    for bl in blur:
        options["blur radius"] = bl
        t = time.time()
        px_des, px_cost, px_des_hist, px_t = px_no_pol(seed, wavelengths, 
                                                               target, pol, 
                                                               layers, options, 
                                                               sim_dtype, geo_dtype, device)
        px_times.append(time.time() - t)
    ...
    # NN + pixel approach
    t = time.time()
    NN_px_des, NN_px_cost, NN_px_des_hist, NN_px_t  = NN_px_no_pol(seed, wavelengths, 
                                                                    target, pol, 
                                                                    layers, options, 
                                                                    sim_dtype, geo_dtype, device)
    NN_px_time = time.time() - t
    ...
    if plot:
        ...
        compare_performances_single_pol_spectral(NN_t, NN_px_t, LMpx_ts, px_ts, target, 
                                                 wavelengths, blur, f"{res_folder}{label}_performance_compare.png")
